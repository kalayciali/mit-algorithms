...
# Algorithms class BST Priority Queue implementation
# BST implementation make it run fast 
# With the assumption that the tree will be balanced
...
class PriorityQueue:
    '''
    BST tree based priority queue implementation
    '''
    def __init__(self):
    ...
    def append(self, transition):
        ...
        new = BSTnode(transition)
        if self.root is None:
            self.root = new
        else:
            node = self.root
            while True:
                if transition < node.key:
                    # go left
                    if node.left == None:
                        node.left = new
                        new.parent = node
